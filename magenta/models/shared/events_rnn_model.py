...
class ConvLSTMCell(tf.nn.rnn_cell.RNNCell):
  ...
  def call(self, x, state):
    ...
    ### having some problems here with input shapes can't be concat
    x = tf.concat([x, h], axis=self._feature_axis)

    # n here I think is the batch size
    n = x.shape[-1].value

    # m is based off the number of filters
    m = 4 * self._filters if self._filters > 1 else 4

    # m and n are used here! After this, with current setup, W should be [16, 64, 64]
    W = tf.get_variable('kernel', self._kernel + [n, m])

    # this is where the actual convolutions haappen
    # uses Kernel, batch size, and 4x(num filter). Looks like right setup
    ...
    # so some rnn stuff is happening here
    # just stores c (hidden state) and h (output) in that order
